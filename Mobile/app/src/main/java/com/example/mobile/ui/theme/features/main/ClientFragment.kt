        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Log.d("CLIENT_ACTIVITY_DEBUG", "O corpo da resposta é nulo? ${clientes == null}")


                    // Acessa a lista de clientes DENTRO do objeto de resposta
                    val listaDeClientes = clientes?.data
                    Log.d("CLIENT_ACTIVITY_DEBUG", "A lista 'data' é nula? ${listaDeClientes == null}")
                    Log.d("CLIENT_ACTIVITY_DEBUG", "Tamanho da lista: ${listaDeClientes?.size}")

                    if (listaDeClientes?.isNotEmpty() == true) {
                        Log.d("CLIENT_ACTIVITY_DEBUG", "Primeiro cliente: ${listaDeClientes.firstOrNull()}")
                    }
                ...
                withContext(Dispatchers.Main) {
                    Log.e("CLIENT_ACTIVITY_DEBUG", "Falha na conexão/parsing: ${e.message}")
                    Toast.makeText(
                        requireContext(),
